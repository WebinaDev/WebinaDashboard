...
export default function UsersListPage() {
  ...
  const usersQ = useQuery({
    ...
    queryFn: () => {
      ...
      return apiFetch<{ items: UserListRow[]; page: number; per_page: number; found: number }>(`users?${p.toString()}`)
    },
    enabled: !isBotMode,
  })
    ...
    <PageShell title={t('users.title')}>
      ...
      <Card className="mb-4 shadow-sm">
        <CardContent className="flex flex-wrap items-end gap-3 pt-6">
          <div className="inline-flex rounded-lg border border-border p-1" role="tablist" aria-label={t('users.botFilterLabel', { defaultValue: 'Bot filter' })}>
