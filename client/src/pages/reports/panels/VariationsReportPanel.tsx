...
export function VariationsReportPanel() {
  const { t, i18n } = useTranslation()
  const store = useStoreCurrency()
  const state = useReportListState()
  const q = useReportListQuery<OrderReportProductProfitRow>('variations', state.filterState.filters, {
    search: state.search,
    page: state.page,
  ...
  const columns: ReportColumn<OrderReportProductProfitRow>[] = [
    { id: 'name', header: t('reports.table.variation'), sortable: true, cell: (r) => r.name },
    {
      id: 'quantity',
  ...
  return (
    <div className="space-y-6">
      <ReportPeriodToolbar filterState={state.filterState} exportSection="variations" />
      {q.isLoading ? (
