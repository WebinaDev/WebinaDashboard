        ...
        {showEmpty ? (
          <div className="space-y-3 rounded-lg border border-dashed px-4 py-6 text-center">
            <p className="text-sm font-medium">
              {traffic.active === false ? t('home.disabled.analytics') : t('home.traffic.emptyTitle')}
            </p>
            <p className="text-muted-foreground text-sm">{t('home.traffic.emptyHint')}</p>
            <p className="text-muted-foreground text-xs">
              {traffic.source === 'wp-statistics'
                ? t('home.traffic.wpStatisticsHint')
                : t('home.traffic.trackerHint')}
            </p>
            <div className="flex flex-wrap items-center justify-center gap-2 pt-1">
              <Button asChild variant="outline" size="sm">
                <Link to="/analytics/settings">{t('home.traffic.openAnalyticsSettings')}</Link>
              </Button>
              {traffic.active === false ? (
                <Button asChild variant="ghost" size="sm">
                  <Link to="/settings">{t('home.disabled.openSettings')}</Link>
                </Button>
              ) : null}
            </div>
          </div>
          ...
          <>
            ...
            <p className="text-muted-foreground text-xs">
              {t('home.traffic.trackerHint')}{' '}
              <Link to="/analytics/settings" className="underline underline-offset-2">
                {t('home.traffic.openAnalyticsSettings')}
              </Link>
            </p>
          </>
        )}
