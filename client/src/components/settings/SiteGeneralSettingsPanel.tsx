...
export function SiteGeneralSettingsPanel() {
  ...
  const initial = useMemo(() => getSsrPage()?.siteGeneral as SiteGeneral | undefined, [])
  ...
  if (!draft && q.isLoading) return <FormSettingsSkeleton cards={2} fieldsPerCard={3} />
  ...
  return (
    <div className="space-y-4 pb-20 sm:pb-0">
      <Card variant="glass">
        <CardHeader>
          <CardTitle className="text-base">{t('settings.site.identityTitle')}</CardTitle>
          <CardDescription>{t('settings.site.identityHint')}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="blogname">{t('settings.site.fieldSiteTitle')}</Label>
            <Input id="blogname" value={draft.blogname} onChange={(e) => setDraft({ ...draft, blogname: e.target.value })} className="w-full" />
          </div>
          <div className="space-y-2">
            <Label htmlFor="blogdescription">{t('settings.site.fieldTagline')}</Label>
            <Input
              id="blogdescription"
              value={draft.blogdescription}
              onChange={(e) => setDraft({ ...draft, blogdescription: e.target.value })}
              className="w-full"
            />
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t('settings.site.technicalTitle')}</CardTitle>
          <CardDescription>{t('settings.site.technicalHint')}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Input
              id="timezone_string"
              value={draft.timezone_string}
              onChange={(e) => setDraft({ ...draft, timezone_string: e.target.value })}
              className="w-full"
            />
          </div>
