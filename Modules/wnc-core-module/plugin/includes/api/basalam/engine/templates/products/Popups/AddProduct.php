...
<div id="basalamAddProductsModal" class="basalam-modal basalam-modal-hidden">
    <div class="basalam-modal-content basalam-modal-max-width-400">
        <span class="basalam-modal-close"> <img class="basalam-modal-img-20" src="<?php echo esc_url(wncBasalamPlugin()->assetsUrl() . '/icons/close.svg'); ?>">
        </span>
        ...
        <?php else: ?>
            <p class="basalam-p wnc_basalam-onboarding-paragraph">آیا مطمئن هستید که می‌خواهید همه محصولات را به باسلام ارسال کنید؟</p>
        <?php endif; ?>
        ...
        <?php if (!$has_active_create_jobs): ?>
            <form method="POST" action="#" id="basalamAddProductsForm">
                ...
                <button type="<?php echo $has_active_create_jobs ? 'button' : 'submit'; ?>"
                    ...
                    <img src="<?php echo esc_url(wncBasalamPlugin()->assetsUrl() . '/icons/submit.svg'); ?>" alt="">
                    ارسال محصولات
                </button>
            </form>
        <?php endif; ?>
        ...
        <?php if ($has_active_create_jobs): ?>
            ...
            <form method="POST" action="<?php echo esc_url(admin_url('admin-post.php')) ?>" id="BasalamCancelCreateJobs">
                ...
                <button type="submit" class="basalam-primary-button basalam-p basalam-danger-button-full">
                    <img class="basalam-danger-button-img" src="<?php echo esc_url(wncBasalamPlugin()->assetsUrl() . '/icons/trash.svg'); ?>">
                    متوقف کردن عملیات
                </button>
            </form>
        <?php endif; ?>
...
<div id="BasalamConfirmModal" class="basalam-modal basalam-modal-display-none">
    <div class="basalam-modal-content basalam-modal-max-width-400">
        <span class="basalam-modal-close">
            <img class="basalam-modal-img-20" src="<?php echo esc_url(wncBasalamPlugin()->assetsUrl() . '/icons/close.svg'); ?>">
        </span>
