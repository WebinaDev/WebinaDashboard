...
use WncBasalam\Admin\Components\SettingPageComponents;

?>
<div id="sync-wnc_basalam-onboarding-orders" class="basalam-action-card basalam-relative">
    ...
    <div class="basalam-action-buttons basalam-action-buttons-col">
        ...
        <form action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="post" class="Basalam-form basalam-form-margin-0">
            ...
            <?php SettingPageComponents::syncStatusOrder(); ?>
            <?php if ($syncStatusOrder == true): ?>
                <button type="submit" class="basalam-danger-button basalam-p basalam-width-fill-available">
                    <img class="basalam-btn-img-20" src="<?php echo esc_url(wncBasalamPlugin()->assetsUrl() . '/icons/unsync.svg'); ?>">
                    توقف همگام‌سازی سفارشات
                </button>
            <?php else: ?>
                <button type="submit" class="basalam-primary-button basalam-p basalam-width-fill-available">
                    <img class="basalam-btn-img-20" src="<?php echo esc_url(wncBasalamPlugin()->assetsUrl() . '/icons/sync.svg'); ?>">
                    همگام‌سازی سفارشات
                </button>
            <?php endif; ?>
        </form>
        ...
        <form action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="post" class="Basalam-form basalam-form-margin-0">
            ...
            <?php SettingPageComponents::renderAutoConfirmOrderButton(); ?>
            ...
            <?php else: ?>
                <button type="submit" class="basalam-primary-button basalam-p basalam-width-fill-available">
                    <img class="basalam-btn-img-20" src="<?php echo esc_url(wncBasalamPlugin()->assetsUrl() . '/icons/order.svg'); ?>">
