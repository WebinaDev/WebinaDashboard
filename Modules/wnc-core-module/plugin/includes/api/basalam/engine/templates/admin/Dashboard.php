...
$settings = wncBasalamSettings()->getSettings();
...
<div class="basalam-container">
    ...
    <?php
    $tokenTemplate = apply_filters('wnc_basalam_token_template', null);
    if ($tokenTemplate) {
        require_once($tokenTemplate);
    } elseif (!$BasalamAccessToken || !$BasalamRefreshToken) {
        require_once(wncBasalamPlugin()->templatePath() . "/admin/main/GetToken.php");
    } else {
        require_once(wncBasalamPlugin()->templatePath() . "/admin/main/Connected.php");
    }
    ?>
</div>