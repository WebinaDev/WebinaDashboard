<?php
use WncBasalam\Services\Products\FetchUnsyncProducts;
use WncBasalam\Admin\Components\ProductListComponents;

defined('ABSPATH') || exit;

$wnc_basalam_sync_status_checker = new FetchUnsyncProducts();
...
$unsync_products = $wnc_basalam_sync_status_checker->getUnsyncBasalamProducts($cursor, $nextCursor);
...
if (!empty($unsync_products)) {
    ...
    <div class="basalam-pagination basalam-pagination-flex">
        <?php if ($hasPrev):
            $prevArgs = ['page' => 'wnc_basalam-show-products'];
        ...
        <?php if (!empty($nextCursor)): ?>
            <?php
            $nextArgs = ['page' => 'wnc_basalam-show-products', 'unsync_cursor' => $nextCursor];
