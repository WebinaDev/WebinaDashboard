...
?>
<div id="sync-wnc_basalam-onboarding-status" class="basalam-status-card">
    ...
    <div class="basalam-sync-status">
        ...
        <form action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="post" class="Basalam-form basalam-form-margin-0">
            ...
            <?php if ($syncStatusProduct == true): ?>
                <button type="submit" class="basalam-danger-button basalam-p">
                    <img class="basalam-img-20" src="<?php echo esc_url(wncBasalamPlugin()->assetsUrl() . '/icons/unsync.svg'); ?>">
                    توقف همگام‌سازی محصولات
                </button>
            <?php else: ?>
                <button type="submit" class="basalam-primary-button basalam-p basalam-btn-small">
                    <img class="basalam-img-20" src="<?php echo esc_url(wncBasalamPlugin()->assetsUrl() . '/icons/sync.svg'); ?>">
                    همگام‌سازی محصولات
                </button>
            <?php endif; ?>
        </form>
        <div class="basalam-info-icon basalam-info-icon-small">
            <a href="https://www.aparat.com/v/vja08ql" target="_blank">
                <img src="<?php echo esc_url(wncBasalamPlugin()->assetsUrl() . "/icons/info-black.svg"); ?>" alt="اطلاعات" class="basalam-img-22 basalam-cursor-pointer">
            </a>
        </div>
