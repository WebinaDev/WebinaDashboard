document.addEventListener("DOMContentLoaded", function () {
  ...
  const isTrackedAdvancedSettingField = (fieldName) =>
    ...
    fieldName.startsWith("wnc_basalam_settings[");
  ...
  const initBulkEditFields = () => {
    const SYNC_BASALAM_BULK_ACTION = "wnc_basalam_bulk_edit";

    const syncTypeVisibility = () => {
      document
        .querySelectorAll('select[name="wnc_basalam_bulk_product_type_action"]')
        .forEach((select) => {
    ...
    const syncPriceChangeVisibility = () => {
      document
        .querySelectorAll('select[name="wnc_basalam_bulk_price_change_action"]')
        .forEach((select) => {
          const field = select
            .closest(".sync-basalam-bulk-edit")
    ...
    document.addEventListener("change", (event) => {
      ...
      if (target.name === "wnc_basalam_bulk_product_type_action") {
        syncTypeVisibility();
      }

      if (target.name === "wnc_basalam_bulk_price_change_action") {
        syncPriceChangeVisibility();
      }
    });
  ...
  var modal = document.getElementById("wnc_basalam_support_modal");
  var btn = document.getElementById("wnc_basalam_support_btn");
  var cancelBtn = document.getElementById("wnc_basalam_cancel_btn");
  ...
  if (starsContainer) {
    ...
    starsContainer.addEventListener('click', function(e) {
      if (e.target.classList.contains('basalam-star')) {
        ...
        document.getElementById('wnc_basalam_rating').value = currentRating;
        updateStars(currentRating);
      }
    });
  }
  ...
  // Remind Later button
  var remindLaterBtn = document.getElementById('wnc_basalam_remind_later_review_btn');
  if (remindLaterBtn) {
    remindLaterBtn.addEventListener('click', function() {
      var nonceEl = document.getElementById('wnc_basalam_remind_later_review_nonce');
      jQuery.ajax({
        url: ajaxurl,
        type: 'POST',
        data: {
          action: 'wnc_basalam_remind_later_review',
          _wpnonce: nonceEl ? nonceEl.value : ''
        },
        success: function() {
          document.getElementById('wnc_basalam_like_alert').style.display = 'none';
        }
      });
    });
  }
  ...
  // Never Remind button
  var neverRemindBtn = document.getElementById('wnc_basalam_never_remind_review_btn');
  if (neverRemindBtn) {
    neverRemindBtn.addEventListener('click', function() {
      var nonceEl = document.getElementById('wnc_basalam_never_remind_review_nonce');
      jQuery.ajax({
        url: ajaxurl,
        type: 'POST',
        data: {
          action: 'wnc_basalam_never_remind_review',
          _wpnonce: nonceEl ? nonceEl.value : ''
        },
        success: function() {
          document.getElementById('wnc_basalam_like_alert').style.display = 'none';
  ...
  // Submit Review form
  var supportForm = document.getElementById('wnc_basalam_support_form');
  if (supportForm) {
    supportForm.addEventListener('submit', function(e) {
      e.preventDefault();
      var nonceEl = document.getElementById('wnc_basalam_submit_review_nonce');
      var ratingEl = document.getElementById('wnc_basalam_rating');
      var commentEl = document.getElementById('wnc_basalam_comment');

      jQuery.ajax({
        url: ajaxurl,
        type: 'POST',
        data: {
          action: 'wnc_basalam_submit_review',
          _wpnonce: nonceEl ? nonceEl.value : '',
          wnc_basalam_rating: ratingEl ? ratingEl.value : '5',
          wnc_basalam_comment: commentEl ? commentEl.value : ''
        },
