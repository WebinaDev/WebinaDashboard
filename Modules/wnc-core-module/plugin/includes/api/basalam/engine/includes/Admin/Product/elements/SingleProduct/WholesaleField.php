...
class WholesaleField
{
    public static function renderCheckbox()
    {
        ...
        wp_nonce_field('wnc_basalam_save_wholesale_action', '_wnc_basalam_wholesale_nonce');
    }

    public static function saveCheckbox($postId)
    {
        if (
            !isset($_POST['_wnc_basalam_wholesale_nonce'])
            || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['_wnc_basalam_wholesale_nonce'])), 'wnc_basalam_save_wholesale_action')
        ) {
            return;
        }
