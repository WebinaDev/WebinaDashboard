...
class MigratorService
{
    ...
    public function migratePayments()
    {
        ...
        $target = $this->wpdb->prefix . 'wnc_basalam_payments';
    ...
    public function migrateOptions()
    {
        ...
        $target = $this->wpdb->prefix . 'wnc_basalam_map_options';

        if ($this->tableExists($source) && $this->tableExists($target)) {
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Legacy-data migration; table identifiers from $wpdb->prefix / core $wpdb properties, not user input.
            $this->wpdb->query("INSERT INTO $target (woo_name, wnc_basalam_name)
    ...
    public function migrateUploadedPhotos()
    {
        ...
        $target = $this->wpdb->prefix . 'wnc_basalam_uploaded_media';
    ...
    public function migratePostMeta()
    {
        ...
        $this->wpdb->query(
            "UPDATE {$this->wpdb->postmeta}
             SET meta_key = REPLACE(meta_key, 'bslm_', 'wnc_basalam_')
        ...
        $this->wpdb->query(
            "UPDATE {$this->wpdb->postmeta}
             SET meta_key = REPLACE(meta_key, 'bsl_', 'wnc_basalam_')
    ...
    public function migrateOptionsRows()
    {
        ...
        $this->wpdb->query(
            "UPDATE {$this->wpdb->options}
             SET option_name = REPLACE(option_name, 'bslm_', 'wnc_basalam_')
        ...
        $this->wpdb->query(
            "UPDATE {$this->wpdb->options}
             SET option_name = REPLACE(option_name, 'bsl_', 'wnc_basalam_')
    ...
    public function migrateActions()
    {
        ...
        $this->wpdb->query(
            $this->wpdb->prepare(
                "UPDATE $source
                 SET hook = REPLACE(hook, 'bslm_', 'wnc_basalam_')
        ...
        $this->wpdb->query(
            $this->wpdb->prepare(
                "UPDATE $source
                 SET hook = REPLACE(hook, 'bsl_', 'wnc_basalam_')
                 WHERE hook LIKE %s AND status = %s",
                '%bsl_%',
                'pending'
            )
    ...
    {
        $settingsRowName = 'wnc_basalam_settings';
    ...
    public function addCreatedAtColumnToUploadedPhoto()
    {
        ...
        $table = $this->wpdb->prefix . 'wnc_basalam_uploaded_photo';
    ...
    {
        $table = $this->wpdb->prefix . 'wnc_basalam_payments';
    ...
    private function removeDuplicateInvoicePayments($table)
    {
        ...
        foreach ($duplicates as $duplicate) {
                ...
                if ($orderId > 0 && function_exists('wc_get_order')) {
                    ...
                    if ($order instanceof \WC_Order) {
                        try {
                            ...
                            \WncBasalam\Logger\Logger::error(
            ...
            \WncBasalam\Logger\Logger::debug(
    ...
    {
        $tableName = $this->wpdb->prefix . 'wnc_basalam_job_manager';
    ...
    {
        $settings = get_option('wnc_basalam_settings', []);
        if (!is_array($settings)) return;
    ...
    public function createUploadedMediaTable()
    {
        ...
        $tableName = $this->wpdb->prefix . 'wnc_basalam_uploaded_media';
    ...
    {
        $legacyTable = $this->wpdb->prefix . 'wnc_basalam_uploaded_photo';
        ...
        $this->wpdb->query(
            "INSERT IGNORE INTO {$targetTable} (type, source_identity, media_id, media_url, created_at)
             SELECT 'photo', CONCAT('attachment:', woo_photo_id), wnc_basalam_photo_id, wnc_basalam_photo_url, COALESCE(created_at, NOW())
             FROM {$legacyTable}
             WHERE woo_photo_id IS NOT NULL"
        );
