...
use WncBasalam\Admin\Product\Validators\ProductOperationValidator;
...
abstract class AbstractProductOperation implements ProductOperationInterface
{
    ...
    {
        $this->validator = $validator ?: wncBasalamContainer()->get(ProductOperationValidator::class);
    }
    ...
    final public function execute(int $product_id, array $args = []): array
    {
        ...
        do_action('wnc_basalam_before_product_operation', $product_id, $args, $operationName);

        do_action("wnc_basalam_before_{$operationName}", $product_id, $args);
        ...
        try {
            ...
            if (!$validation) {
                ...
                $result = apply_filters('wnc_basalam_product_operation_validation_error', $result, $product_id, $operationName);
            ...
            $result = apply_filters('wnc_basalam_product_operation_result', $result, $product_id, $args, $operationName);

            $result = apply_filters("wnc_basalam_{$operationName}_result", $result, $product_id, $args);

            do_action('wnc_basalam_after_product_operation', $result, $product_id, $args, $operationName);

            do_action("wnc_basalam_after_{$operationName}", $result, $product_id, $args);
            ...
            return apply_filters('wnc_basalam_product_operation_exception', $result, $product_id, $operationName);
        }
    }
