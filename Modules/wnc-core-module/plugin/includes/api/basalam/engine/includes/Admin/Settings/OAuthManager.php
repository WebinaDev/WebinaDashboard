...
class OAuthManager
{
    ...
    const OAUTH_STATE_TRANSIENT = 'wnc_basalam_oauth_state_';
    ...
    {
        $oauthDataUrl = apply_filters('wnc_basalam_oauth_data_url', Endpoints::HAMSALAM_OAUTH_DATA);
        $defaultClientId = apply_filters('wnc_basalam_oauth_default_client_id', 779);
        $defaultRedirectUri = apply_filters('wnc_basalam_oauth_default_redirect_uri', Endpoints::HAMSALAM_OAUTH_TOKEN);
    ...
    public static function saveOauthData()
    {
        ...
        if (! current_user_can('manage_options') || ! self::verifyOauthState()) {
            wp_die(
                esc_html__('درخواست نامعتبر است.', 'webinaconnector'),
                esc_html__('خطای امنیتی', 'webinaconnector'),
                ['response' => 403]
            );
        }
        ...
        $extraData = apply_filters('wnc_basalam_oauth_save_extra_data', []);

        if ($isVendor == 'false') {
            $data = [SettingsConfig::IS_VENDOR => false];
            $data = apply_filters('wnc_basalam_oauth_non_vendor_data', $data, $vendorId, $accessToken, $refreshToken, $extraData);
    ...
    public function getOAuthUrls()
    {
        ...
        $scopes = apply_filters('wnc_basalam_oauth_scopes', "vendor.product.write vendor.parcel.write customer.profile.read vendor.profile.read vendor.parcel.read vendor.profile.write customer.chat.read customer.chat.write customer.identity.read");
