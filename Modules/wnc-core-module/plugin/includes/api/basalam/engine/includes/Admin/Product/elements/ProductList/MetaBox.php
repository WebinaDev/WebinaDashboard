...
class MetaBox
{
    public static function registerMetaBox()
    {
        ...
        add_meta_box(
            'wnc_basalam_single_product_manage_box',
            'تنظیمات باسلام <span title="' . esc_attr($statusTooltip) . '" style="background:' . esc_attr($statusColor) . ';width: 16px;height: 16px;border-radius: 100%;">
                <img src="' . esc_url(wncBasalamPlugin()->assetsUrl() . "/icons/info.svg") . '" alt="" class="basalam-img-15">
                </span>',
    ...
    public static function renderMetaBox($post)
    {
        ...
        if ($productStatus == 'publish') {
            if ($basalamProductSyncStatus == 'pending') {
                ...
                echo '<p class="basalam-p basalam-font-12">دسترسی های لازم دریافت نشده است ، ابتدا دسترسی ها را <a href="/wp-admin/admin.php?page=wnc_basalam" target="_blank">دریافت</a> نمایید.</p>';
            } else {
                ...
                if ($basalamProductStatus && $syncBasalamProductId) {
                    ...
                    require_once wncBasalamPlugin()->templatePath("products/ConnectButton.php");
                }
            }
        } else {
        ...
        <div id="wnc_basalam_category_id" class="basalam-p__small basalam--hidden">
            </div>';
        ...
        echo '<div id="wnc_basalam_category_attributes" class="basalam-p__small basalam--hidden">
