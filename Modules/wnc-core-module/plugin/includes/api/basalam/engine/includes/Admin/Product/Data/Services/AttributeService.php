...
use WncBasalam\Utilities\TextConverter;
use WncBasalam\Services\Products\GetCategoryAttr;
...
class AttributeService
{
    ...
    {
        $isEnabled = wncBasalamSettings()->getSettings(SettingsConfig::PRODUCT_ATTRIBUTE_SUFFIX_ENABLED);

        if ($isEnabled !== 'yes') return null;

        $attributeName = trim(wncBasalamSettings()->getSettings(SettingsConfig::PRODUCT_ATTRIBUTE_SUFFIX_PRIORITY));
    ...
    public function generateDescription($product): string
    {
        ...
        if (wncBasalamSettings()->getSettings(SettingsConfig::ADD_SHORT_DESC_TO_DESC_PRODUCT) === 'yes') {
        ...
        if (wncBasalamSettings()->getSettings(SettingsConfig::ADD_ATTR_TO_DESC_PRODUCT) === 'yes') {
        ...
        foreach ($wooAttributes as &$wooAttribute) {
            foreach ($mappedOptions as $mappedOption) {
                if (trim($mappedOption['woo_name']) === trim($wooAttribute['title'])) {
                    $wooAttribute['title'] = $mappedOption['wnc_basalam_name'];
                    break;
                }
            }
        }
    ...
    {
        global $wpdb;
        $categoryOptionsManager = new \WncBasalam\Admin\Product\Category\CategoryOptions($wpdb);
