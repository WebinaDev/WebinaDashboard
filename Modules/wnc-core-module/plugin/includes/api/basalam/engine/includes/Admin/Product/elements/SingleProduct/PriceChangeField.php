...
<?php
        wp_nonce_field('wnc_basalam_save_price_change_action', '_wnc_basalam_price_change_nonce');
    }

    public static function saveField($postId)
    {
        if (
            !isset($_POST['_wnc_basalam_price_change_nonce'])
            || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['_wnc_basalam_price_change_nonce'])), 'wnc_basalam_save_price_change_action')
        ) {
            return;
        }
