...
namespace WncBasalam\Actions\Controller\TicketActions;

use WncBasalam\Actions\Controller\ActionController;
use WncBasalam\Services\TicketServiceManager;
use WncBasalam\Utilities\TicketExtraInfoFormatter;
use WncBasalam\Utilities\TicketAccessData;
    ...
    public function __invoke()
    {
        ...
        try {
            ...
            $redirectUrl = $this->createdTicketId > 0
                ? admin_url('admin.php?page=wnc_basalam_ticket&ticket_id=' . $this->createdTicketId)
                : admin_url('admin.php?page=wnc_basalam_new_ticket');
            wp_safe_redirect($redirectUrl);
            exit();
        }

        $redirectUrl = admin_url('admin.php?page=wnc_basalam_ticket&ticket_id=' . $ticketId);
        if (wp_doing_ajax()) {
