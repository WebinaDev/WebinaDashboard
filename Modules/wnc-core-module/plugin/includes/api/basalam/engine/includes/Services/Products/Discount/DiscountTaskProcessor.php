...
class DiscountTaskProcessor
{
    ...
    public function __construct(
        ...
        $this->discountManager = $discountManager ?: wncBasalamContainer()->get(DiscountManager::class);
        $this->taskModel = $taskModel ?: wncBasalamContainer()->get(DiscountTaskModel::class);
        $this->jobManager = $jobManager ?: wncBasalamContainer()->get(JobManager::class);
        $this->settingsAccessor = wncBasalamSettings();
    }

    public function processDiscountTasks()
    {
        $job = $this->jobManager->getJob(['job_type' => 'wnc_basalam_discount_tasks', 'status' => 'pending']);
    ...
    {
        $jobExists = $this->jobManager->getCountJobs(['job_type' => 'wnc_basalam_discount_tasks', 'status' => ['pending', 'processing']]);
        $discountReductionPercent = absint($this->settingsAccessor->getSettings(SettingsConfig::DISCOUNT_REDUCTION_PERCENT));

        if ($jobExists === 0) $this->jobManager->createJob('wnc_basalam_discount_tasks', 'pending');
