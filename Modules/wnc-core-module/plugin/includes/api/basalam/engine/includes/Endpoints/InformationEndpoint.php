...
use WncBasalam\Services\Api\RequestStatusTracker;
...
class InformationEndpoint
{
    ...
    public static function checkPermissions($request)
    {
        ...
        if (empty($token)) {
            return new \WP_Error(
                'wnc_basalam_missing_status_token',
                'Token is required.',
                ['status' => 403]
            );
        }
        ...
        if (!self::validateStatusEndpointToken($token)) {
            return new \WP_Error(
                'wnc_basalam_invalid_status_token',
                'توکن معتبر نیست.',
                ['status' => 403]
            );
        }
    ...
    public static function getPluginStatus($request)
    {
        ...
        return rest_ensure_response([
            'domain'                => get_site_url(),
            'woosalam_version'      => wncBasalamPlugin()->getVersion(),
