...
namespace WncBasalam\Admin\Announcement;

use WncBasalam\Admin\Components\AnnouncementComponents;
use WncBasalam\Services\FetchAnnouncements;

defined('ABSPATH') || exit;

class AnnouncementCenter
{
    public const MARK_SEEN_ACTION = 'wnc_basalam_mark_announcements_seen';
    public const FETCH_PAGE_ACTION = 'wnc_basalam_fetch_announcements_page';
    private const SEEN_META_KEY = 'wnc_basalam_seen_announcements';
    ...
    public static function shouldLoadAnnouncement(): bool
    {
        ...
        if ($page === '' || !is_admin() || !current_user_can('manage_options') || !wncBasalamSettings()->hasToken()) {
            return false;
        }

        if (str_starts_with($page, 'wnc_basalam')) return true;
        return false;
    }
