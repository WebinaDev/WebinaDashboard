...
use WncBasalam\Admin\Product\elements\SingleProduct\PriceChangeField;
use WncBasalam\Admin\Product\Utils\ProductUnits;
...
class BulkEdit
{
    ...
    /**
     * Runs on admin_action_wnc_basalam_bulk_edit, which passes no arguments,
    ...
    public function save(): void
    {
        ...
        if (!isset($_REQUEST['wnc_basalam_bulk_product_type_action']) && !isset($_REQUEST['wnc_basalam_bulk_price_change_action'])) {
            return;
        }
    ...
    {
        $action = isset($_REQUEST['wnc_basalam_bulk_product_type_action'])
            ? sanitize_text_field(wp_unslash($_REQUEST['wnc_basalam_bulk_product_type_action']))
            : 'keep';
        ...
        if ($action === 'yes') {
            ...
            $unit = isset($_REQUEST['wnc_basalam_bulk_product_unit'])
                ? sanitize_text_field(wp_unslash($_REQUEST['wnc_basalam_bulk_product_unit']))
                : '6304';
            $quantity = isset($_REQUEST['wnc_basalam_bulk_product_value'])
                ? sanitize_text_field(wp_unslash($_REQUEST['wnc_basalam_bulk_product_value']))
                : '1';
    ...
    {
        $action = isset($_REQUEST['wnc_basalam_bulk_wholesale_action'])
            ? sanitize_text_field(wp_unslash($_REQUEST['wnc_basalam_bulk_wholesale_action']))
            : 'keep';
    ...
    {
        $action = isset($_REQUEST['wnc_basalam_bulk_price_change_action'])
            ? sanitize_text_field(wp_unslash($_REQUEST['wnc_basalam_bulk_price_change_action']))
            : 'keep';
        ...
        $value = isset($_REQUEST['wnc_basalam_bulk_price_change_value'])
            ? sanitize_text_field(wp_unslash($_REQUEST['wnc_basalam_bulk_price_change_value']))
            : '';
