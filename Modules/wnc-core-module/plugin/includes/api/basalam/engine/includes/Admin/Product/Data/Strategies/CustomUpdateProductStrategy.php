...
use WncBasalam\Services\Products\UpdateProductVariationsService;
...
class CustomUpdateProductStrategy implements DataStrategyInterface
{
    ...
    {
        $data = apply_filters('wnc_basalam_product_payload', null, $product, $handler, 'custom_update');
        if (!is_array($data)) {
            $data = [];
        }
    ...
    {
        $setting = wncBasalamSettings()->getSettings($fieldKey);
