 ...
 */
final class WNC_Torob_Webhook_Queue_Runner
{
    ...
    const FALLBACK_INACTIVE_ERROR = 'wnc_torob_runner_fallback_inactive';
    const INVALID_TOKEN_ERROR = 'wnc_torob_invalid_runner_token';
    const WEBHOOK_NOT_READY_ERROR = 'wnc_torob_product_webhook_not_ready';

    private const START_TIMESTAMP_ERROR = 'wnc_torob_runner_start_timestamp_error';
    private const COMPLETION_TIMESTAMP_ERROR = 'wnc_torob_runner_completion_timestamp_error';
    ...
    public function __construct(WNC_Torob_Webhook_Queue_Services $queue_services, ?bool $wp_cron_disabled = null)
    {
    ...
    {
        if (!array_key_exists(WNC_Torob_Webhook_Queue_Services::CRON_INTERVAL, $schedules)) {
            $schedules[WNC_Torob_Webhook_Queue_Services::CRON_INTERVAL] = [
                'interval' => WNC_Torob_Webhook_Queue_Services::PROCESSING_INTERVAL_SECONDS,
    ...
    {
        if (!WNC_Torob_Options::isProductPageWebhookReady()) {
            return;
        }
    ...
    {
        if (!$this->is_fallback_active()) {
            WNC_Torob_Options::resetProductPageWebhookRunnerLock();
    ...
    public function run_fallback(string $lock_token)
    {
        ...
        try {
            ...
            $cooldown_until = $started_at + WNC_Torob_Webhook_Queue_Services::PROCESSING_INTERVAL_SECONDS;
            $this->process_queue();
    ...
    {
        return WNC_Torob_Options::getProductPageWebhookRunnerLastCompletedAt();
    }
    ...
    public function get_next_run_timestamp(): ?int
    {
        if (!$this->is_wp_cron_disabled()) {
            $next_run_timestamp = wp_next_scheduled(WNC_Torob_Webhook_Queue_Services::CRON_HOOK);
        ...
        $last_started_at = WNC_Torob_Options::getProductPageWebhookRunnerLastStartedAt();
        if ($last_started_at === null) {
            return null;
        }

        return $last_started_at + WNC_Torob_Webhook_Queue_Services::PROCESSING_INTERVAL_SECONDS;
    }
    ...
    {
        WNC_Torob_Webhook_Queue_Services::clear_cron_schedule();
        WNC_Torob_Options::resetProductPageWebhookRunner();
    }
    ...
    {
        if (!WNC_Torob_Options::setProductPageWebhookRunnerLastStartedAt(time())) {
    ...
    {
        if (!WNC_Torob_Options::setProductPageWebhookRunnerLastCompletedAt(time())) {
    ...
    {
        $stored_timestamp = get_option(WNC_Torob_Options::PRODUCT_PAGE_WEBHOOK_RUNNER_LAST_STARTED_AT_OPTION, false);
        if ($stored_timestamp !== false) {
            return;
        }

        WNC_Torob_Options::initializeProductPageWebhookRunner();
    }
    ...
    {
        return $this->is_wp_cron_disabled() && WNC_Torob_Options::isProductPageWebhookEnabled();
    }
    ...
    private function configure_wp_cron(): void
    {
        ...
        add_action(WNC_Torob_Webhook_Queue_Services::CRON_HOOK, [$this, 'run_cron']);

        if (!wp_next_scheduled(WNC_Torob_Webhook_Queue_Services::CRON_HOOK)) {
            wp_schedule_event(time(), WNC_Torob_Webhook_Queue_Services::CRON_INTERVAL, WNC_Torob_Webhook_Queue_Services::CRON_HOOK);
        }
    }
    ...
    private function unregister_wp_cron_hooks(): void
    {
        ...
        remove_action(WNC_Torob_Webhook_Queue_Services::CRON_HOOK, [$this, 'run_cron']);
    }
    ...
    private function get_next_due_timestamp(): ?int
    {
        ...
        return $last_started_at === null ? null : $last_started_at + WNC_Torob_Webhook_Queue_Services::PROCESSING_INTERVAL_SECONDS;
    }
    ...
    {
        $existing_lock = get_option(WNC_Torob_Options::PRODUCT_PAGE_WEBHOOK_RUNNER_LOCK_OPTION, null);
        ...
        if ($existing_lock === null) {
            $acquired = add_option(WNC_Torob_Options::PRODUCT_PAGE_WEBHOOK_RUNNER_LOCK_OPTION, $runner_lock, '', false);
        } else {
            $acquired = WNC_Torob_Options::replaceProductPageWebhookRunnerLock($existing_lock, $runner_lock);
        }
    ...
    {
        $runner_lock = get_option(WNC_Torob_Options::PRODUCT_PAGE_WEBHOOK_RUNNER_LOCK_OPTION, null);
        ...
        return WNC_Torob_Options::replaceProductPageWebhookRunnerLock($runner_lock, $claimed_lock) ? $claimed_lock : null;
    }
    ...
    private function complete_runner_claim(array $claimed_lock, ?int $cooldown_until): void
    {
        ...
        WNC_Torob_Options::replaceProductPageWebhookRunnerLock($claimed_lock, $cooldown_lock);
    }
