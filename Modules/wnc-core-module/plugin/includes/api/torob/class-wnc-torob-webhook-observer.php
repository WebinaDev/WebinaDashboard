 ...
 */
class WNC_Torob_Webhook_Observer
{
    const WATCHED_PROPS = [
        'name',
        'slug',
    ...
    public function __construct(WNC_Torob_Webhook_Queue_Services $queue_services)
    {
     ...
     */
    public function build_for_product(WC_Product $product, ?string $page_url = null): ?WNC_Torob_Webhook_Item
    {
        ...
        if ($resolved_page_url === null) {
            ...
            $resolved_page_url = WNC_Torob_Extraction_Utils::get_page_url($page_product);
        }

        return new WNC_Torob_Webhook_Item((string) WNC_Torob_Extraction_Utils::get_page_unique($page_product), $resolved_page_url);
    }

    /**
     * Build an item for the product page represented by a product or variation post ID.
     */
    public function build_for_post_id(int $post_id): ?WNC_Torob_Webhook_Item
    {
     ...
     */
    public function build_removal_for_product_post(\WP_Post $post): ?WNC_Torob_Webhook_Item
    {
        ...
        if (!is_string($page_url) || $page_url === '') {
            $page_url = WNC_Torob_Extraction_Utils::get_page_url($product);
        }
