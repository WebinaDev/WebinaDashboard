 ...
 */
class WNC_Torob_Webhook_Queue_Services
{
    const CRON_HOOK = 'wnc_torob_process_pending_product_page_webhooks';
    ...
    /** @var array<int, WNC_Torob_Webhook_Item> */
    ...
    public static function clear_pending_queue(): void
    {
        ...
        WNC_Torob_Webhook_Queue_Repo::clear();
    }
    ...
    {
        return WNC_Torob_Webhook_Queue_Repo::count_pending();
    }
    ...
    {
        return WNC_Torob_Webhook_Queue_Repo::get_paginated_webhook_items($limit, $offset);
    }

    /**
     * Queue a resolved webhook item to be persisted at shutdown.
     */
    public function queue_webhook_item(?WNC_Torob_Webhook_Item $item): void
    {
        if ($item === null) {
            return;
        }
     ...
     */
    public function upsert_queued_item(WNC_Torob_Webhook_Item $item, ?string $date_modified = null): bool
    {
        return WNC_Torob_Webhook_Queue_Repo::upsert_queued_product(
    ...
    public function process_pending_product_page_webhooks(): void
    {
        ...
        try {
            $rows = WNC_Torob_Webhook_Queue_Repo::get_ready_batch(
            ...
            foreach ($rows as $row) {
                $items[] = new WNC_Torob_Webhook_Item((string) $row->product_id, (string) $row->page_url);
            }

            $send_result = WNC_Torob_HTTP::send_product_page_webhook_items(...$items);

            if ($send_result->is_success()) {
                WNC_Torob_Webhook_Queue_Repo::delete_processed_rows($rows);
                return;
            }

            if ($send_result->get_status_code() === 401) {
                error_log('Product page webhook queue services: Unauthorized, resetting token');
                WNC_Torob_Options::resetToken();
    ...
    private function claim_processing_lock(): ?array
    {
        ...
        $existing_lock = get_option(WNC_Torob_Options::PRODUCT_PAGE_WEBHOOK_PROCESSING_LOCK_OPTION, null);

        if ($existing_lock === null) {
            return add_option(WNC_Torob_Options::PRODUCT_PAGE_WEBHOOK_PROCESSING_LOCK_OPTION, $processing_lock, '', false)
                ? $processing_lock
                : null;
        }
        ...
        return WNC_Torob_Options::replaceProductPageWebhookProcessingLock($existing_lock, $processing_lock)
            ? $processing_lock
            : null;
    }
    ...
    {
        WNC_Torob_Options::deleteProductPageWebhookProcessingLock($processing_lock);
    }
}
