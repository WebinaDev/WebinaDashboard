 ...
 */
final class WNC_Torob_Lifecycle
{
    ...
    {
        $stored_version = WNC_Torob_Options::getReportedVersion();
        if ($stored_version === WNC_VERSION) {
            return;
        }
    ...
    private static function send(string $event): void
    {
        ...
        try {
            WNC_Torob_HTTP::send_lifecycle_event($event, WNC_VERSION);
    ...
    {
        if (defined('WNC_TOROB_DISABLE_LIFECYCLE_EVENTS') && WNC_TOROB_DISABLE_LIFECYCLE_EVENTS) {
            return false;
        }

        return (bool) apply_filters(
            'wnc_torob_lifecycle_event_reporting_enabled',
            !self::is_local_site_url(WNC_Torob_Site_Data::url()),
            $event
        );
    }
    ...
    {
        try {
            WNC_Torob_Options::setReportedVersion(WNC_VERSION);
        } catch (Throwable $error) {
